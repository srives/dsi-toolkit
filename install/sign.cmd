...
:DEBUG
	...
	if not exist "%SOURCEPATH%\src\bin\x%BIT%\Debug (Revit %1)\DSIRevitToolkit.dll" goto :RELEASE
	echo SIGN Debug DLL for %1
	"%signTool%" sign /td SHA256 /fd SHA256 /f %signFile% /p %pw% /tr http://timestamp.digicert.com/ "%SOURCEPATH%\src\bin\x%BIT%\Debug (Revit %1)\DSIRevitToolkit.dll"

:RELEASE
    if (%WHICH%)==(Debug) goto :EOF
	if not exist "%SOURCEPATH%\src\bin\x%BIT%\Release (Revit %1)\DSIRevitToolkit.dll" goto :EOF
	echo SIGN Release DLL for %1
	"%signTool%" sign /td SHA256 /fd SHA256 /f %signFile% /p %pw% /tr http://timestamp.digicert.com/ "%SOURCEPATH%\src\bin\x%BIT%\Release (Revit %1)\DSIRevitToolkit.dll"
